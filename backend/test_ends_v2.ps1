...
function Test-Endpoint {
    ...
    try {
        ...
        if ($statusCode -eq 200) {
            $msg = "$green OK ($statusCode) $reset"
        } else {
            $msg = "$yellow UNKNOWN ($statusCode) $reset"
        }
    }
    catch {
        ...
        if ($statusCode -eq 503) {
            $msg = "$yellow NOT READY ($statusCode) $reset"
        } else {
            $msg = "$red FAILED ($statusCode) $reset"
        }
    }
    ...
    # Salva no arquivo COM CORES ANSI
    "$label$msg" | Out-File -FilePath "resultado_endpoints.txt" -Append -Encoding utf8

    # Se houver erro detalhado, tenta capturar
    if ($msg -like "*FAILED*") {
        try {
            ...
            if ($errorJson.error) {
                $errMsg = "$red   Error: $($errorJson.error)$reset"
                Write-Host $errMsg
                $errMsg | Out-File -FilePath "resultado_endpoints.txt" -Append -Encoding utf8
            }
        } catch {}
    }
}
