/**
 * Test Script for Trading Strategies (PERMISSIVE MODE)
 * Tests Iron Condor and Iron Butterfly with VERY LOW THRESHOLDS to validate full cycle
 * 
 * ⚠️  WARNING: This is for TESTING ONLY!
 * Thresholds are intentionally set very low to force entry and validate the complete flow.
 * DO NOT use these settings in production!
 * 
 * USAGE:
 * 1. Make sure backend is running on port 3300
 * 2. Run: node test-strategies-permissive.js
 * 
...
async function main() {
  console.log('\n' + '█'.repeat(80));
  console.log('TRADING STRATEGY TEST SUITE (PERMISSIVE MODE)');
  console.log('⚠️  WARNING: Using very low thresholds for testing only!');
  console.log('█'.repeat(80) + '\n');
  ...
  try {
    ...
    // Test Iron Condor (PERMISSIVE)
    console.log('\n⚠️  Testing Iron Condor with PERMISSIVE settings...');
    console.log('   (Thresholds intentionally lowered to validate full cycle)\n');
    await testStrategy('iron_condor', {
      accountBalance: 10000,
      riskPercent: 2,
      minIVRank: 0,          // ⚠️  Accept any IV Rank
      maxIVRank: 100,
      minDTE: 1,             // ⚠️  Accept any DTE
      maxDTE: 365,
      minVolume: 0,          // ⚠️  No volume requirement
      minOI: 0,              // ⚠️  No OI requirement
      maxDeltaTotal: 1.0,    // ⚠️  Accept high delta
      minTheta: -999,        // ⚠️  Accept any theta
      profitTarget: 0.5,
      stopLoss: 2.0
    }, marketData, indicators);

    // Test Iron Butterfly (PERMISSIVE)
    console.log('\n⚠️  Testing Iron Butterfly with PERMISSIVE settings...');
    console.log('   (Thresholds intentionally lowered to validate full cycle)\n');
    await testStrategy('iron_butterfly', {
      ...
      maxDeltaTotal: 1.0,    // ⚠️  Accept high delta
      minTheta: -999,        // ⚠️  Accept any theta
      atmTolerance: 0.1,     // ⚠️  10% ATM tolerance
      profitTarget: 0.5,
      stopLoss: 2.0
    }, marketData, indicators);
